...
- 既有 170 个批准输入、G08 manifest、真实左右镜像和核心不变量是 Formation 改动的回归基线。
...
- F00 与 F00-R2 已完成人眼验收并封存；行为提交为 `fb9bea56e9b14c7532e6684acbdb2b39e61c470e`。
- 下一批准增量是合并完成 F01–F03：少量结构化起手 → 已验证域内带 seed 的有界随机 → 预先锁定的 Formation held-out。三段必须保留内部停止门，前一段失败不得继续后一段。
- F03 manifest 必须在揭示结果前锁定；失败后不得按 case ID 修补同一批输入。完成 F01–F03 后先交用户验收，不自动开始 A 阶段。
- 除 F02 已批准的有界 seed 采样外，不增加任意随机位置、拖拽、自动选边、手动控制、新战术、投篮结果、更多人数、5v5、ML/RL 或生产化。
- F02 的随机只允许来自 F01 已证明的合法 Formation 域；它不等于任意半场站位。自动选掩护侧、自动 anchor 和安全退出属于后续 A 阶段。
