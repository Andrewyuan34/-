...
- 当前只批准 Formation 的 F00：从一个固定、合法的偏移起手形成一次挡拆，再接入旧内核。
- 标准 F00 动画已通过用户首次人眼判断；提交前纠偏的实现及自动门也已通过，当前只等待两个交接瞬间的短回放复验。
- 纠偏只覆盖 O1/O5 联合就绪、D1 篮筐侧责任、语义验收门、固定模板/最小拓扑拒绝和最小信息所有权；不是推倒重做，也不是开始 F01。
- 未经用户重新批准，不增加随机位置、拖拽、自动选边、手动控制、新战术、投篮结果、更多人数、5v5、ML/RL 或生产化。
- 纠偏自动门通过后只让用户复验掩护设稳与 next-tick 交接；用户确认后才单独 commit/push，然后停止。
